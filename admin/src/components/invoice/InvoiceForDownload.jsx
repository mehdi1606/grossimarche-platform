...
import logo from "@/assets/img/logo/logo-primary.png";

/**
 * The order's PDF invoice, as the back-office prints it. Same document the customer downloads
 * from the storefront (store-without-stripe/src/components/invoice/InvoiceForDownload.js):
 * one order, one invoice, whoever opens it.
 *
 * No Font.register here, deliberately. This file used to pull Open Sans and DejaVu from two
 * CDNs; @react-pdf/renderer resolves those at render time, so a blocked or slow CDN left the
 * download spinning for good. The built-in Helvetica carries French accents perfectly well.
 */

/** Who issues the invoice. Change it here and on the storefront copy of this file. */
const SELLER = {
  name: "Market Food",
...
// The API stores the status in English; the invoice prints it in French (same wording as
// components/table/Status.jsx).
const STATUS_FR = {
  Pending: "En attente",
...
const InvoiceForDownload = ({ data, currency, getNumberTwo, showDateFormat }) => {
  ...
  const meta = [
    ...
    ["Date", showDateFormat ? showDateFormat(data?.createdAt) : ""],
  ];
  const status = STATUS_FR[data?.status] || data?.status || "-";
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View>
          ...
          {data?.user_info?.email ? (
            <Text style={styles.seller}>{data.user_info.email}</Text>
          ) : null}
        ...
        <Text style={styles.thanks}>Merci {name}, votre commande a bien été reçue.</Text>
