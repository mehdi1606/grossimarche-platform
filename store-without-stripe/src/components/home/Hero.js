...
// Keys rather than sentences: this runs at module load, where no hook exists, so the words
// are resolved at render time instead of being frozen in French here.
const PROMISES = ["perk_tiers", "perk_cod", "perk_delivery"];
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_24rem] xl:gap-16">
          ...
          <div className="max-w-2xl">
            <span
              ...
              style={fade()}
            >
              <FiShoppingBag /> {t("home.hero_badge")}
            </span>
            ...
            <ul className="mt-8 flex flex-wrap gap-x-6 gap-y-2" style={fade(0.22)}>
              {PROMISES.map((key) => (
                <li key={key} className="flex items-center gap-2 text-sm text-emerald-100/90">
